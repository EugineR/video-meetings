...
- `apps/web` — Next.js frontend (TypeScript, App Router) on Tailwind CSS v4 and HeroUI v3: `/register` and `/login`, a home page (`/`) listing the signed-in user's meetings, `/meetings/{id}` with the meeting's details and its recording (a video or audio player, or an uploader), showing the recording's transcription status and transcript text once ready, and `/profile`/`/profile/edit` for viewing and editing the signed-in user's display name, password and avatar. Auth is client-side only, via a `localStorage`-stored JWT. See `apps/web/CLAUDE.md`.
