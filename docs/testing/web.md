...
| `/`              | spinner until the auth check resolves, redirect to `/login` with no session, "Recent"/"All meetings" (desktop table / mobile list, at the `lg` breakpoint), upload from a recent card and from a table/list row's overflow menu, the "+ Create meeting" modal (validation, success prepending the new meeting to both, cancel/close without creating), the mobile drawer opening from the top bar's menu button and closing via both the backdrop and its close button, and signing out from the sidebar/drawer's account overflow menu                                         |
...
Any change touching `(app)/layout.tsx`, `(dashboard)/layout.tsx` or `(workspace)/layout.tsx`
is exercised on `/meetings/{id}`, `/profile` and `/profile/edit` too, confirming they still
render inside the original `AppShell` (header, gradient background) unaffected by the
dashboard shell `/` uses — the two are separate route groups precisely so a `/` change cannot
leak into the other three routes.
