...
- **Naming Convention**: Descriptive names (e.g., `add-user-login`, `fix-header-bug`, `refactor/auth-module`).
  - _Optional_: You may use prefixes like `feat/`, `fix/`, `chore/` for clarity, but strictly structured names like Gitflow are not mandatory unless specified by team policy.
- **Lifecycle**:
  ...
  4.  Deleted after merging to `master`.
