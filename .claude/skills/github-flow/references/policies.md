...
**Rule**: Always pull the latest `master` before branching.
...
- **Merge Strategy**: Squash and Merge is often preferred to keep the `master` history clean, but Merge Commits are also valid depending on team preference.
- **Deployment**: Merging to `master` automatically triggers a deployment to production (or staging -> production).
