...
- `src/app.module.ts` — root module. Imports `ConfigModule` (global), `PrismaModule`, `UsersModule`, `AuthModule`, `MeetingsModule`; registers a global `ValidationPipe` (`whitelist: true, transform: true`) via `APP_PIPE`
...
- **Unit tests** (`pnpm test` / `pnpm test:watch` / `pnpm test:cov`): Jest config lives inline in `package.json` (`rootDir: "src"`, `testRegex: ".*\\.spec\\.ts$"` — matches `*.spec.ts` colocated with the code under `src`). No unit tests exist yet — the only one (`app.controller.spec.ts`, for the scaffold hello-world controller) was removed along with `AppController`/`AppService`, and no replacement unit tests have been written for the `auth`/`users`/`meetings` CQRS handlers. `passWithNoTests: true` is set in the Jest config so `pnpm test` exits 0 with zero suites instead of failing with "no tests found" — remove that flag once real unit tests are added. Run a single unit test file once one exists: `pnpm test -- <name>.spec.ts` (Jest pattern match); a single test case: `pnpm test -- -t "test name"`.
