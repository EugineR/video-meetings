...
- `src/app/page.tsx` — home page (`/`), a client component (`'use client'`). On mount it checks `getStoredUser()` (`src/lib/auth.ts`); if there's no valid stored user it redirects (`router.replace`) to `/login`, otherwise it renders `AppHeader` (welcoming the user by email, with a sign-out button — `clearAccessToken` + redirect to `/login`) followed by the user's meetings, fetched via `getMeetings()` (`src/lib/api.ts`, `GET /meetings`): a "Recent meetings" `Card` listing `MeetingRow`s for the 3 most recently dated meetings (client-side sort/slice — the API returns the full unsorted list, no `limit`/`sort` params exist), shown only when there is at least one meeting, and an "All meetings" `Card` listing every meeting as a `MeetingRow` with a `Total: N` count. Shows a HeroUI `Spinner` while the auth check or the meetings fetch is pending, and an inline error (from `ApiError`) if the meetings fetch fails.
...
- `src/components/` — one component per file, grouped into subfolders by kind rather than kept flat:
  ...
  - `components/meetings/MeetingRow.tsx` — a single meeting's card row (title, calendar-icon-prefixed formatted date, users-icon-prefixed comma-joined participant list), used by `page.tsx` for both the "Recent" and "All meetings" lists; takes an optional `highlighted` prop for the blue-tinted "Recent meetings" styling.
  - `components/icons/` — one small inline SVG icon component per file (`EyeIcon.tsx`/`EyeOffIcon.tsx` for password visibility toggles, `VideoCameraIcon.tsx` for the app logo, `CalendarIcon.tsx`/`UsersIcon.tsx` for meeting metadata), re-exported through `icons/index.ts` so call sites import from `@/components/icons` rather than each file individually.
- `src/lib/api.ts` — thin `fetch` wrapper for calling `apps/api`: reads the base URL from `NEXT_PUBLIC_API_URL` (env var, see below), exposes `registerUser`, `loginUser`, and `getMeetings` (the last attaches `Authorization: Bearer <token>` via `getAccessToken()` from `src/lib/auth.ts`), and throws `ApiError` (with the HTTP status and the API's `message`) on non-2xx responses.
